...
class UserControllerTest {
    ...
    void testGetList() throws Exception {
        ...
        assertEquals(3, userList.size()); // 件数が3件である

        assertEquals(1, userList.get(0).getId()); // idとnameの検証
        assertEquals("キラメキ太郎", userList.get(0).getName());

        assertEquals(2, userList.get(1).getId()); // idとnameの検証
        assertEquals("キラメキ次男", userList.get(1).getName());

        assertEquals(3, userList.get(2).getId()); // idとnameの検証
